...
class GeminiInterface(BaseInterface):
    ...
    def generate_response(self, message: str, channel_id: int, node_short_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            if node_short_name:
                logging.debug(f"Generating response for private chat with {node_short_name}")
            ...
            # Admin channel
                ...
                logging.debug("Generating response for admin channel")
                response = self.admin_chat.send_message(message)
                response_text = response.text
            
            # Public channel
            elif channel_id == 0:  # public_channel_number
                logging.debug("Generating response for public channel")
