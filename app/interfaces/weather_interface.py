...
from core.constants import (
    USER_AGENT_STRING,
    WEATHER_ALERTS_CACHE_SECONDS,
    WEATHER_API_RATE_LIMIT_SECONDS,
    WEATHER_CONDITIONS_CACHE_SECONDS,
    WEATHER_FORECAST_CACHE_SECONDS,
    WEATHER_POINTS_CACHE_SECONDS,
)
...
class WeatherGovInterface(APIInterface):
    ...
    def __init__(self, user_agent: str = USER_AGENT_STRING, config_manager=None):
        """
        ...
        super().__init__(
            ...
            cache_duration_seconds=WEATHER_FORECAST_CACHE_SECONDS,
            default_headers=headers,
            rate_limit_delay=WEATHER_API_RATE_LIMIT_SECONDS,
        )
    ...
    def get_forecast(self, latitude: float, longitude: float) -> dict[str, Any]:
        ...
        try:
            ...
            if response.get("success"):
                ...
                self.logger.debug(
                    f"Cached new forecast data for {self.city}, {self.state} ({latitude}, {longitude})"
                )
                return forecast_data
            else:
                self.logger.error(
                    f"Error fetching forecast: {response.get('error', 'Unknown error')}"
                )
            ...
            return {
                "error": "Internet connection unavailable. Please check your network and try again."
            }
        ...
        try:
            ...
            if cached_points:
                ...
                if not response.get("success"):
                    self.logger.error(
                        f"Error fetching location details: {response.get('error', 'Unknown error')}"
                    )
                    return

                metadata = response["data"]
                # Cache for 24 hours — points data for a coordinate never changes
                self._cache_data(points_cache_key, metadata, WEATHER_POINTS_CACHE_SECONDS)
    ...
    def update_alerts(self, latitude: float, longitude: float) -> dict[str, Any]:
        ...
        try:
            ...
            if not alerts_data:
                ...
                if response.get("success"):
                    ...
                    self._cache_data(zone, alerts_data, WEATHER_ALERTS_CACHE_SECONDS)
                else:
                    self.logger.error(
                        f"Error fetching alerts: {response.get('error', 'Unknown error')}"
                    )
                    return []
            else:
        ...
        try:
            ...
            for feature in features:
                ...
                self.logger.debug(
                    f"Processed alert ID {alert_id}: {self.current_alerts[alert_id].get('headline', 'No headline')}"
                )

            self.expired_alerts = {
                k: v for k, v in self.previous_alerts.items() if k not in self.current_alerts
            }

            # Update new alerts
            self.new_alerts = {
                k: v for k, v in self.current_alerts.items() if k not in self.previous_alerts
            }

            # Update updated alerts
            self.updated_alerts = {
                k: v
                for k, v in self.current_alerts.items()
                if k in self.previous_alerts and v != self.previous_alerts[k]
            }
    ...
    def get_current_conditions(self, latitude: float, longitude: float) -> dict[str, Any]:
        ...
        try:
            ...
            if not response.get("success"):
                self.logger.error(
                    f"Error fetching stations: {response.get('error', 'Unknown error')}"
                )
                return {"error": response.get("error", "Unknown error")}
            ...
            if not obs_response.get("success"):
                self.logger.error(
                    f"Error fetching observations: {obs_response.get('error', 'Unknown error')}"
                )
                return {"error": obs_response.get("error", "Unknown error")}

            conditions_data = obs_response["data"]
            self._cache_data(cache_key, conditions_data, WEATHER_CONDITIONS_CACHE_SECONDS)
