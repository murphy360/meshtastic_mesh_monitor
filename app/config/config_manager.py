...
class ConfigManager:
    ...
    def __init__(self, config_file_path: str | None = None):
        ...
        if config_file_path is None:
            ...
            config_file_path = os.path.join(
                parent_dir, "config_files", "config.json"
            )  # /app/config_files/config.json (Docker)
            self.logger.debug(f"Using config file at: {config_file_path}")
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        try:
            if os.path.exists(self.config_file_path):
                ...
                self.logger.warning(
                    f"Configuration file not found at {self.config_file_path}, Not using config file."
                )
    ...
    def get_node_short_name() -> str:
        ...
        return os.environ.get(
            "NODE_SHORT_NAME", ""
        )  # Empty string triggers use of radio's actual short name
    ...
    def get_node_long_name() -> str:
        ...
        return os.environ.get(
            "NODE_LONG_NAME", ""
        )  # Empty string triggers use of radio's actual long name
    ...
    def add_rss_feed(
        self, feed_id: str, name: str, url: str, enabled: bool = True, check_interval_hours: int = 1
    ):
        """
