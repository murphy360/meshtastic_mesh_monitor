...
class BaseInterface(ABC):
    ...
    def _cache_data(self, cache_key: str, data: Any, duration_seconds: int | None = None) -> None:
    ...
    def _clear_cache(self, cache_key: str | None = None) -> None:
        """Clear cache data."""
    ...
    def _handle_api_error(self, error: Exception, context: str) -> dict[str, Any]:
        ...
        self.logger.error(f"Error in {context}: {error!s}")
        return {
            "success": False,
            "error": str(error),
            "context": context,
...
class FeedInterface(PollingInterface):
    ...
    def __init__(
        ...
        cache_duration_seconds: int = 3600,
        default_poll_interval_seconds: int = 3600,
        discard_initial_items: bool = True,
    ):
        """
    ...
    def add_feed(
        self, feed_id: str, feed_url: str, poll_interval_seconds: int | None = None
    ) -> None:
        ...
        return {
            ...
            "feeds": {
                feed_id: {
                    ...
                    "poll_interval": self.poll_intervals.get(
                        feed_id, self.default_poll_interval
                    ).total_seconds(),
                }
