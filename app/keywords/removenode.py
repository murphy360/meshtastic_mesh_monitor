    ...
    def handle(self, interface, packet):
        ...
        if "decoded" not in packet or "payload" not in packet["decoded"]:
            ...
            self.message_sender.send_message(
                interface,
                "No decoded payload found in packet for removenode keyword.",
                channel,
                to_id,
            )
            return
        ...
        if len(args) < 2:
            self.logger.error(
                "[handle] Removenode keyword requires at least one argument (node identifier). Usage: removenode <shortname>"
            )
            self.message_sender.send_message(
                interface, "Usage: removenode <shortname>", channel, to_id
            )
            return
        ...
        self.logger.info(
            f"[handle] Found {len(nodes)} nodes matching identifier '{node_identifier}'"
        )
        log_message = ""
        if len(nodes) > 0:
            for node in nodes:
                self.logger.info(
                    f"[handle] Removing node {node['user']['shortName']} - {node['num']}"
                )
            ...
            self.logger.info(
                f"[handle] Node {node_identifier} not found in my database. Unable to remove."
            )
            self.message_sender.send_message(
                interface,
                f"Node {node_identifier} not found. Unable to remove from my database.",
                channel,
                to_id,
            )
