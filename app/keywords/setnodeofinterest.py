...
class SetnodeofinterestKeyword(KeywordHandler):
    ...
    def handle(self, interface, packet):
        ...
        if len(args) < 3:
            self.logger.error(
                "[handle] Invalid command format. Usage: setnodeofinterest <node_identifier> <true/false>"
            )
            self.message_sender.send_message(
                interface,
                "Invalid command format. Usage: setnodeofinterest <node_identifier> <true/false>",
                channel,
                to_id,
            )
            return
        ...
        if set_as_interest not in ["true", "false"]:
            self.logger.error(
                f"[handle] Invalid argument for set_as_interest: {set_as_interest}. Must be 'true' or 'false'."
            )
            self.message_sender.send_message(
                interface,
                f"Invalid argument for set_as_interest: {set_as_interest}. Must be 'true' or 'false'.",
                channel,
                to_id,
            )
            return
        ...
        if set_as_interest == "true":
            ...
            self.message_sender.send_message(
                interface, f"{node_identifier} is now a node of interest", channel, to_id
            )
            # sitrep.log_message_sent("node-of-interest-set")
        else:
            self.logger.info(f"[handle] {node_identifier} is no longer a node of interest")
            self.message_sender.send_message(
                interface, f"{node_identifier} is no longer a node of interest", channel, to_id
            )
