...
class TextHandler(BaseHandler):
    ...
    def on_receive(
        self, packet: dict, interface: object, public_channel_number: int | None = None
    ) -> None:
        """
        ...
        self.logger.info(
            f"[on_receive_text] onReceiveText called for node {node_short_name} - {from_node_num}"
        )
        localNode = interface.getNode(LOCAL_NODE_ID)
        if node is None:
            self.logger.warning(
                f"[on_receive_text] onReceiveText: Node {from_node_num} not found, skipping text handling."
            )
            return
        if self._is_local_node(interface, from_node_num):
            self.logger.info(
                f"[on_receive_text] Received text from local node {node_short_name} - {from_node_num}. Ignoring packet."
            )
            return

        if "toId" in packet and "decoded" in packet:
            self.logger.info(
                f"[on_receive_text] Processing text packet from {node_short_name} - {from_node_num}"
            )
            to_id = packet["to"]
            ...
            self.logger.debug(
                f"Portnum: {portnum}, Payload: {payload}, Bitfield: {bitfield}, Message: {message_string}"
            )

            # log to_id and local node num
            self.logger.info(
                f"[on_receive_text] Message to ID: {to_id}, Local node num: {localNode.nodeNum}"
            )
            ...
            if to_id == localNode.nodeNum:  # Message sent directly to local node
                ...
                self.logger.info(
                    f"Direct message received from {node_short_name}: '{message_string}' on channel {channelId}"
                )
                if self.check_keywords(interface, packet):
                    self.logger.info(
                        f"Keyword detected and handled in direct message from {node_short_name}. No further action taken."
                    )
                else:
                    self.message_sender.send_direct_reply(
                        interface, message_string, channelId, packet["from"]
                    )
            elif "channel" in packet:  # Message sent to a channel
                channelId = int(packet["channel"])
                self.logger.info(
                    f"Message on channel {channelId} from {node_short_name}: '{message_string}'"
                )
            ...
            self.logger.info("Unable to process text packet")
    ...
    def check_keywords(self, interface: object, packet: dict) -> None:
        ...
        keyword_files = [
            f[:-3] for f in os.listdir(keywords_dir) if f.endswith(".py") and not f.startswith("__")
        ]
        for keyword in keyword_files:
            if potential_keywords[0] == keyword:
                ...
                try:
                    ...
                    spec = importlib.util.spec_from_file_location(
                        keyword, os.path.join(keywords_dir, f"{keyword}.py")
                    )
            ...
            if keyword in potential_keywords:
                ...
                self.logger.info(
                    f"Keyword '{keyword}' detected in message but not at start. No action taken."
                )
        return False
