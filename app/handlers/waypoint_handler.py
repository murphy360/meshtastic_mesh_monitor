...
class WaypointHandler(BaseHandler):
    ...
    def on_receive(self, packet: dict, interface: object, admin_channel_number: int) -> None:
        ...
        self.logger.info(
            f"[on_receive_waypoint] onReceiveWaypoint called for node {node_short_name} - {from_node_num}"
        )
        if node is None:
            self.logger.warning(
                f"[on_receive_waypoint] Node {from_node_num} not found, skipping waypoint handling."
            )
            return
        ...
        self.logger.info(
            f"Waypoint ID: {id}, Latitude: {latitude}, Longitude: {longitude}, Expire: {expire}, Name: {name}, Description: {description}"
        )
        if expire == 1:
            self.logger.info(f"Waypoint {name} is expired")
            self.message_sender.send_llm_message(
                interface,
                f"Waypoint {name} is expired",
            ...
            self.message_sender.send_llm_message(
                interface,
                f"Waypoint {name}, {description} expires at {expire_time}",
                admin_channel_number,
                BROADCAST_DESTINATION,
            )
