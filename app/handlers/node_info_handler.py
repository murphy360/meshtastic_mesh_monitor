...
def on_receive_node_info(packet, interface):
    ...
    logger.info(f"[on_receive_node_info] onReceiveNodeInfo called for node {packet['from']}")
    ...
    logger.info(f"[on_receive_node_info] onReceiveNodeInfo completed for node {node_short_name} - {from_node_num}")
