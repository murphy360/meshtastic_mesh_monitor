    ...
    def execute(self) -> bool:
        try:
            ...
            if expired_alerts and len(expired_alerts) > 0:
                self.logger.info(
                    f"⏰ SENDING {len(expired_alerts)} expired weather alert notifications"
                )
                expired_message = "The following weather alerts are no longer active:\n"
                for _alert_id, alert_data in expired_alerts.items():
                    expired_message += f"- {alert_data['event']}: {alert_data['headline']}\n"
                self.message_sender.send_llm_message(
                    tcp_interface, expired_message, alert_channel, BROADCAST_DESTINATION
                )
                if self.sitrep:
            ...
            if updated_alerts and len(updated_alerts) > 0:
                self.logger.info(
                    f"📝 SENDING {len(updated_alerts)} updated weather alert notifications"
                )
                for _alert_id, alert_data in updated_alerts.items():
                    alert_message = (
                    ...
                    self.message_sender.send_llm_message(
                        tcp_interface, alert_message, alert_channel, BROADCAST_DESTINATION
                    )
                    if self.sitrep:
            ...
            if new_alerts and len(new_alerts) > 0:
                ...
                for _alert_id, alert_data in new_alerts.items():
                    alert_message = (
                        f"NEW WEATHER ALERT\n"
                        f"Type: {alert_data['event']}\n"
                        f"Severity: {alert_data['severity']}\n"
            ...
            total_active = (
                len(weather_interface.current_alerts)
                if hasattr(weather_interface, "current_alerts")
                else 0
            )
